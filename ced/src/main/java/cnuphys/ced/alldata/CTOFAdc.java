...
/** Read-through access to the current {@code CTOF::adc} bank. */
public final class CTOFAdc {

    public static final String BANK_NAME = "CTOF::adc";
    private static final CTOFAdc INSTANCE = new CTOFAdc(
            () -> DataWarehouse.getInstance().getBank(BANK_NAME));

    private final Supplier<DataBank> bankSupplier;

    private CTOFAdc(Supplier<DataBank> bankSupplier) { this.bankSupplier = bankSupplier; }

    static CTOFAdc forTesting(Supplier<DataBank> bankSupplier) { return new CTOFAdc(bankSupplier); }

    public static CTOFAdc getInstance() { return INSTANCE; }
    ...
    public void addFeedback(int row, List<String> feedbackStrings) {
        feedbackStrings.add(String.format("$cyan$CTOF adc %d time %6.3f order %d",
