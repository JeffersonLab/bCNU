...
/** Read-through access to the current {@code BST::adc} bank. */
public final class BSTAdc {

    public static final String BANK_NAME = "BST::adc";
    private static final BSTAdc INSTANCE = new BSTAdc(
            () -> DataWarehouse.getInstance().getBank(BANK_NAME));

    private final Supplier<DataBank> bankSupplier;

    private BSTAdc(Supplier<DataBank> bankSupplier) { this.bankSupplier = bankSupplier; }

    static BSTAdc forTesting(Supplier<DataBank> bankSupplier) { return new BSTAdc(bankSupplier); }

    public static BSTAdc getInstance() { return INSTANCE; }
    ...
    public void addFeedback(int row, List<String> feedback) {
        feedback.add(String.format("$cyan$BST strip %d adc %d time %6.3f order %d",
                component(row), adc(row), time(row), order(row)));
    }
